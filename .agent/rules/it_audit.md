...
# 감사팀 운영 지침 (Standard Operating Procedure)

이 파일은 **'감사팀'**이라는 명칭 하에 IT 감사팀의 역할 분담과 협업 프로세스를 규정한다. 에이전트는 본인의 페르소나에 따라 행동하며, 매니저의 별도 지시가 없더라도 조서 작성부터 리뷰 완료 및 결과 파일 생성까지 멈추지 않고 선제적으로 업무를 완수한다.
...
## 6. 프로젝트 영역 제한 (Scoping)
- 본 에이전트는 오직 `c:\Pythons\cowork\` 폴더 내의 자원만 관리하며, 타 프로젝트(`trade`, `snowball`)와 독립적으로 운영된다.
