...
```
python cowork/select_top_10.py --source [선택한 source_file명] 실행해줘
```

완료 확인: `cowork/pool_context.json` 파일이 생성되고 섹터별 종목 수가 출력되면 다음 단계로 진행.
...
완료 확인: `cowork/sector_rankings.json` 파일이 생성되고 모든 섹터가 포함되어 있으면 다음 단계로 진행.
...
```
python cowork/audit_save.py cowork/sector_rankings.json --type sector 실행해줘
```
...
```
cowork/pool_context.json 과 cowork/sector_rankings.json 삭제해줘
```
